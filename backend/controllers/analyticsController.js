...
const Application = require("../models/Application");
const { countDocuments } = require("../models/SavedJob");
...
exports.getEmployerAnalytics = async (req, res) => {
   try{
      ...
      //=== counts ===
      const totalActiveJobs = await Job.countDocuments({ company: compnayId, isClosed: false});
      const jobs = await Job.find({ company: compnayId}).select("_id").lean();
      ...
      //=== Data === 
      const recentJobs = await Job.find({ company: compnayId})
      .sort({ createdAt: -1})
      .limit(5)
      .select("title location type createdAt isClosed");

      const recentAppliations = await Application.find({
         job: { $in: jobIds},
      })
      .sort({ createdAt: -1})
      .limit(5)
