...
const NavigationItem = ({ item, isActive, onClick, iSCollapsed}) => {
   ...
   return <button 
     ...
     className={`w-full flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-all duration-200 group ${
      ...
      />
      {!iSCollapsed && <span className="ml-3 truncate">{item.id}</span>}
   </button>
}
...
const DashboardLayout = ({ activeMenu, children}) => {
   ...
   return (
      <div className="flex h-screen bg-gray-50">
         ...
         >
            {/* company logo */}
            <div className="flex items-center h-16 border-b border-gray-200 px-4 sm:px-6">
               {!sidebarCollapsed ? (
                  <Link className="flex items-center space-x-3" to ='/'>
                     <div className="h-8 w-8 bg-gradient-to-br from-blue-600 to-blue-700 rounded-lg flex items-center justify-center">
                        <Briefcase className="h-5 w-5 bg-white" />
                     </div>
                     <span className="text-gray-900 font-bold text-xl">TalentSphere</span>
                  </Link>
               ) : (
         ...
         <div className={`flex-1 min-w-0 flex flex-col transition-all duration-300 ${
            ...
            {/* top navabar */}
            <header className="bg-white/80 backdrop-blur-sm border-b border-gray-200 h-16 flex items-center justify-between px-3 sm:px-4 lg:px-6 sticky top-0 z-30">
